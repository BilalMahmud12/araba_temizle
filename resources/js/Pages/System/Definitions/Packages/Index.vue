<template>
    <div class="h-full">
        <app-head title="Service Packages" />
        <Navigation/>
        <PackagesNav />
        <!--datatable Heading-->
        <div class="app-container py-3">
            <div class="px-4 py-3 bg-white border border-gray-200 flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-lg text-[#1a8f36]">Service Packages</span>
                </div>
                <div class="search ml-auto">
                    <search-input v-model="params.search"/>
                </div>
                <div class="create ml-4">
                    <Link :href="route('system.definitions.packages.create')" as="button" type="button" class="btn-primary flex items-center">
                ...
                <data-table :columns="columns" :sort-key="sortKey" :sort-orders="sortOrders" v-on:sort="sort">
                    <template v-slot:datatable-body>
                        ...
                        <tr v-for="record in records.data" :key="record.id">
                            ...
                            <td>{{ record.name }}</td>
                            <td>{{ record.package_category.name }}</td>
                            <td>{{ record.service_contract.name }}</td>
                            <td>{{ record.price }} ₺</td>

                            <td class="flex items-center justify-center">
                                <Link :href="route('system.definitions.packages.show', record.id)" as="button" type="button">
...
export default {
    ...
    data() {
        ...
        let columns = [
            ...
            { name: 'active', label: 'Status', width: '5%' },
            { name: 'name', label: 'Package Name', width: '25%' },
            { name: 'package_category_id', label: 'Package Category', width: '15%' },
            { name: 'service_contract_id', label: 'Contract', width: '15%' },
            { name: 'price', label: 'Price', width: '10%' },
    ...
    watch: {
        params: {
            handler() {
                this.$inertia.get(this.route('system.definitions.packages.index'), this.params, { replace: true, preserveState: true });
            },
            deep: true
        }
    },
