<template>
    <div class="h-full">
        ...
        <div class="app-container">
            <div class="px-6 py-8 px-6 py-8 bg-white border border-gray-200">
                <form @submit.prevent="submit">
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <textarea-input id="notes" label="Notes" rows="6" v-model="form.content" />
                    </div>
                </form>
            </div>
        </div>
...
export default {
    ...
    setup(props) {
        ...
        const form = useForm({
            ...
            is_active: props.contract.is_active === 1,
            name: props.contract.name,
            content: props.contract.content,
            remember: true
        })
        function submit() {
            form.post(route('system.definitions.contracts.update', form.id))
        }
        function destroy() {
            form.delete( route('system.definitions.contracts.destroy', form.id),  { preserveScroll: true })
        }
