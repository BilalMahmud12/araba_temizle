<template>
    <div class="h-full">
        <app-head title="Update Service District" />
        <Navigation />
        <ServiceSpotsNav />
        <div class="app-container py-3">
            <div class="px-4 py-3 bg-white border border-gray-200 flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-lg text-[#1a8f36]">Update Service District :   {{ form.name }}</span>
                </div>
...
export default {
    ...
    setup(props) {
        ...
        const form = useForm({
            ...
            is_active: props.district.is_active === 1,
            name: props.district.name,
            notes: props.district.notes,
            remember: true
        })
        function submit() {
            form.post(route('system.definitions.spots.districts.update', form.id), form)
        }
        function destroy() {
            form.delete( route('system.definitions.spots.districts.destroy', form.id),  { preserveScroll: true })
        }
